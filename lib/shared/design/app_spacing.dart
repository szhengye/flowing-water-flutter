...
/// 间距系统 —— 4px 基础单位。
///
/// 组织借自 listening-king 的 `AppSpacing`(私有构造 + `static const` 刻度 +
/// `EdgeInsets` 预设 + `num` 扩展),换成了供应商后台需要的中性刻度。
/// 这是「刻度菜单」:各区域按自己的密度从中挑值。
///
/// 直接采用自 spike `spikes/visual-direction/dart-spike/lib/tokens/app_spacing.dart`
/// (wayfinder 08:已就绪,原样采用)。
class AppSpacing {
  AppSpacing._();
